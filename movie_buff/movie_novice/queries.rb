...
def below_average_years
  ...
  Movie.where("score < 5").select("yr, COUNT(*) AS bad_movies").group(:yr).order("bad_movies DESC")
end
...
def uma_movies
  ...
  Movie.select("movies.id, title, yr").joins(:actors).where("name = 'Uma Thurman'").order("yr")
end
